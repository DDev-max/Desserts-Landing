...
export function Header() {
  ...
  return (
    <header
      ...
      <button className={styles.header_menuBtn} ref={btnMenuRef} aria-label="Navigation menu" aria-expanded={menuVisible[0]} aria-controls="navMenu" onClick={() => toggleMenu({ menuIdx: 0, setMenuVisible })}>
        {
          menuVisible[0]
      ...
      <span onClick={() => toggleMenu({ menuIdx: 0, setMenuVisible })} className={`${styles.header_navBg} ${menuVisible[0] ? styles["header_navBg--visible"] : ""}`}></span>
      ...
      <nav id="navMenu" className={`${styles.header_nav} ${menuVisible[0] ? styles["header_nav--visible"] : ""}`}>
        ...
        <ul className={styles.header_nav_menu}>
          ...
          <li
            onMouseEnter={() => { handleEnterLeave({ enter: true, isMouse, menuVisible, setMenuVisible }) }}
            onMouseLeave={() => { handleEnterLeave({ enter: false, isMouse, menuVisible, setMenuVisible }) }}
            className={`${styles.header_nav_menu_item} ${menuVisible[1] ? styles["header_nav_menu_item--extended"] : ""}`}
          >

            <Link
              onClick={() => { if (isMouse.current == false) toggleMenu({ menuIdx: 1, setMenuVisible }) }}
